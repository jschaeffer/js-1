...
//        console name:'stdout', layout:pattern(conversionPattern: '%d{MM-dd-yyyy HH:mm:ss} %-5p %c %x >>%m%n')
        ...
        rollingFile name: 'rollingFileError', layout: pattern(conversionPattern: '%d{MM-dd-yyyy HH:mm:ss} %-5p %c %x >>%m%n'), threshold: Level.ERROR, maxFileSize: "10MB", maxBackupIndex: 15, file: '/opt/tcserver/instances/cm_40/logs/error.log' 
