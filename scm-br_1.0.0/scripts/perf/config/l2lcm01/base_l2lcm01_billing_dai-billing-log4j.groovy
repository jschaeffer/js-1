...
log4j = {
    ...
    appenders {
        'null' name: 'stacktrace' //disable stacktrace file

        console name: 'stdout', layout: pattern(conversionPattern: '%d{MM-dd-yyyy HH:mm:ss} %-5p %c %x >>%m%n'), threshold: Level.INFO

        rollingFile name: 'rollingFileError', layout: pattern(conversionPattern: '%d{MM-dd-yyyy HH:mm:ss} %-5p %c %x >>%m%n'), threshold: Level.ERROR, maxFileSize: "10MB", maxBackupIndex: 15, file: '/opt/tcserver/instances/billing/logs/error.log'
    }
