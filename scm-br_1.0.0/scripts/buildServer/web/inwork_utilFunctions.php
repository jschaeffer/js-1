...
function getTagDropDown($PROJECT, $FORM_ITEM_NAME, $MULTIPLE_SELECT) {
    $SCM_SCRIPTS_DIR = "/opt/build/scm/scripts/buildServer/groovy";
        //echo $SCM_SCRIPTS_DIR;
    $SOURCE_INFO = "source /opt/cvbuild/.bash_profile 2>&1";
    // get an array for each tag
    $tags = array();
    $getTagCmd = 'bash -c "'. $SOURCE_INFO . ' 2>&1 && cd ' .$SCM_SCRIPTS_DIR . ' 2>&1 && bin/eval.sh groovy scm.groovy -c GetTagsForProject --log_level=INFO --project=' . $PROJECT . ' --path_to_project=/opt/checkouts"';
    exec($getTagCmd . ' | egrep -e "^r_*"', $tags);
    usort($tags, 'lodWLevelCompare');
//    $tags = array_reverse($tags);
    array_unshift($tags,'HEAD');

    if($MULTIPLE_SELECT == 'true') {
        $output = '<select name="' . $FORM_ITEM_NAME . '" ' . 'multiple="multiple" size="10">';
    }
    else {
        $output = '<select name="' . $FORM_ITEM_NAME . '">';
    }
    foreach ($tags as $tag) {
        $tag = str_replace('/','',$tag);
        $output = $output . '<option value="' . $tag . '">' . $tag . '</option>';
    }
    $output = $output. '</select>';
    return $output;
}
