...
if (isset($_POST['refresh_button']))
 {
   $file = 'cmd_out.txt';
   $cmdtoout = 'shell_exec(java -jar /opt/tools/tmp/jenkins-cli.jar -s http://cvbuild.cv.infra:7001 buildWithParameters?token=TOKEN_NAME&SelEnv=${targ} SelectSCMEnv-Refresh)';
   file_put_contents($file, $cmdtoout, FILE_APPEND | LOCK_EX);
   shell_exec("java -jar /opt/tools/tmp/jenkins-cli.jar -s http://cvbuild.cv.infra:7001 buildWithParameters?token=TOKEN_NAME&SelEnv=${targ} SelectSCMEnv-Refresh");
 }
...
print "<tr><td bgcolor=${back_col} colspan=7 align=left>
 <form method=POST>
   <input type=submit class=styled-button-2 button name=refresh_button value=\"Refresh\"</button>";

print "$tab$tab<a href=\"../deployLogs/${targ}.html\"><input type=submit class=styled-button-2 value=\"Recent ${targ} Deployments\" /></a>$tab
...
//       $resurl="<input type=submit class=styled-button-3 button name=restart_button value=\"Restart Me\"</button>";
