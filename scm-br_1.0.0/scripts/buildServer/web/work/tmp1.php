...
<body>
<?php foreach ($envs as $targ) {
 print "Working on $targ<br>";
 foreach ($components as $comp) {
   $comp_dir="${targ}/${comp}<br>";
   print "Component Dir : $comp_dir<br>";
?>
  ...
  <tr>
   <td colspan="6">
    <table width="100%" border="0" cellpadding="3" cellspacing="0" id="tbl1" class="tbl">
     <tr bgcolor="#EEEEEE">
      <td width="25%"><b><u>${comp}</b></u></td>
      <td width="25%"><b><u>Current Version/<br>TechOps Pkg. Download</b></u></td>
      <td width="20%"><b><u>Update Avail?</b></u></td>
     ...
     //  $cmloc=shell_exec("ssh tcserver@dvappdai02 grep Implementation-Version /opt/tcserver/instances/cm/webapps/Dynamic-Ad-Insertion-cm/META-INF/MANIFEST.MF | cut -d : -f 2 | tr -d ' '  |tr -d '\r'"); 
      ...
      <?php 
        if ($cmdev > $cmloc) {
             ...
             $recentchg = "<a href=\"http://cvbuild.cv.infra:7001/view/DAI-CM/job/DAI-CampaignMgmt-PackageDeploy-4.0.0/changes\">Changes</a>";
        }
        else {
             $updavail = "No";
             $recentchg ="";
      ...
      <?php 
//        if ($cmdev > $cmloc) {
//             $updavail = "Yes<br><a href=\"java -jar tmp/jenkins-cli.jar -s http://cvbuild.cv.infra:7001 build dai_scm-PackageDeploy-4.0.0\">Update to $cmdev?</a>";
//             $recentchg = "<a href=\"http://cvbuild.cv.infra:7001/view/DevInt%20Env/job/DAI-CampaignMgmt-PackageDeploy-3.0.0/changes\">Changes</a>";
//        }
//        else {
//             $updavail = "No";
//             $recentchg ="";
...
//       ?>
      ...
      <td><a href="http://cvbuild.cv.infra/releaseTars/Dynamic-Ad-Insertion-cm/Dynamic-Ad-Insertion-cm_<?php echo "$cmloc" ?>.tar"><?php echo $cmloc ?></a></td>
...
/*
        if ($adsdev > $adsloc) {
             ...
             $recentchg = "<a href=\"http://cvbuild.cv.infra:7001/view/DAI-DecEng/job/DAI-DecisionEngine-PackageDeploy-3.0.0/changes\">Changes</a>";
        }
        else {
             $updavail = "No";
             $recentchg ="";
...
*/
       ?>
      <td><a href="http://cvbuild.cv.infra/releaseTars/Dynamic-Ad-Insertion-engine/Dynamic-Ad-Insertion-engine_<?php echo "$adsloc" ?>.tar"><?php echo $adsloc ?></a></td>
      <td><?php echo "$updavail"?></td>
      <td><?php echo "$recentchg"?></td>
...
/*
        if ($cipdev > $ciploc) {
             ...
             $recentchg = "<a href=\"http://cvbuild.cv.infra:7001/view/DAI-CIP/job/dai-cip-feedback-PackageDeploy-3.0.0/changes\">Changes</a>";
        }
        else {
             $updavail = "No";
             $recentchg ="";
...
*/
       ?>
      <td><a href="http://cvbuild.cv.infra/releaseTars/dai-cip-feedback/dai-cip-feedback_<?php echo "$ciploc" ?>.tar"><?php echo $ciploc ?></a></td>
      <td><?php echo "$updavail"?></td>
      <td><?php echo "$recentchg"?></td>
...
/*
        if ($smsidev > $smsiloc) {
             ...
             $recentchg = "<a href=\"http://cvbuild.cv.infra:7001/view/DAI_SMSI/job/dai-smsi-PackageDeploy-3.0.0/changes\">Changes</a>";
        }
        else {
             $updavail = "No";
             $recentchg ="";
...
*/
       ?>
      <td><a href="http://cvbuild.cv.infra/releaseTars/dai-smsi/dai-smsi_<?php echo "$smsiloc" ?>.tar"><?php echo $smsiloc ?></a></td>
      <td><?php echo "$updavail"?></td>
      <td><?php echo "$recentchg"?></td>
...
/*
        if ($sadmindev < $sadminloc) {
             $updavail = "Yes<br><a href=`java -jar /opt/tools/tmp/jenkins-cli.jar -s http://cvbuild.cv.infra:7001 build dai_scm-PackageDeploy-4.0.0`>Update to $sadmindev?</a>\"";
             $recentchg = "<a href=\"http://cvbuild.cv.infra:7001/view/DevInt%20Env/job/smsi-admin-PackageDeploy-3.0.0/changes\">Changes</a>";
        }
        else {
...
*/
       ?>
      <td><a href="http://cvbuild.cv.infra/releaseTars/smsi-admin/smsi-admin_<?php echo "$sadminloc" ?>.tar"><?php echo $sadminloc ?></a></td>
      <td><?php echo "$updavail"?></td>
      <td><?php echo "$recentchg"?></td>
