...
if ( !isset($_POST['project']) ) {
	...
	echo 'to scmteam@canoe-ventures.com to have a jenkins build setup for you. The project dropdown values are refreshed <br>';
	echo 'every 5 minutes.<br>';
    echo '</div>';

}

elseif ( !isset($_POST['brtype']) ) {

    echo '<div align="center">';
    echo '<form method="post" onsubmit="return validateForm()" name="brType">';
    echo '<table class="details">';
    echo '<tr>';
    echo '<th colspan="2">Select a branch point type from Project ' . $_POST['project'] . '</th>';
    echo '</tr>';
    echo '<tr>';
    echo '<td><Input type = 'Radio' Name ='brtype' value= 'tag'>Tag</td>';
    echo '<td><Input type = 'Radio' Name ='brtype' value= 'branch'>Branch</td>';
    echo '</tr>';
    echo '</table>';
    echo '<input type="submit" value="brtype"/>';
