...
#tr -d -c '"\n' < ${NEW_VERSION} | awk '{ print length; }'
#tr -d -c '"\n' < ${CURRENT_VERSION} | awk '{ print length; }'
DelimNm1=$(echo $CURRENT_VERSION|awk -F. '{print NF-1}')
DelimNm2=$(echo $NEW_VERSION|awk -F. '{print NF-1}')

if [ $DelimNm1 != $DelimNm2 ]; then
   echo "Branched detected! Please seek SCM assistance!"
   echo "-1"
   exit
fi
