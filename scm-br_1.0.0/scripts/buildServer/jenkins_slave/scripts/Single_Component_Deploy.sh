...
if [[ -z ${7} ]] ; then
  ...
  echo "Component=$1 DB_Rebuild=$2 VERSION=$VERSION Clear_Logs=$3 DeployTarg=$4 BUIILD_TAG=$5 DeployUser=$6"
else
  echo "Version supplied - ${3}"
  export Component=${1}
  export DB_Rebuild=${2}
  export Version=${3}
  export VERSION=${3}
  ...
  export BUILD_TAG=${6}
  export DeployUser=${7}
  echo "Component=$1 DB_Rebuild=$2 VERSION=$3 Clear_Logs=$4 DeployTarg=$5 BUIILD_TAG=$6 DeployUser=$7"
fi
...
#    fnRebuildCheck
  ...
  tar -xvf ${Component}_${VERSION}.tar liquibase/cisys
  cd liquibase/cisys
...
#  executeCommand "scp cvbuild@cvbuild.cv.infra:/opt/build/releaseTars/${Component}/${ARCHIVE} /tmp"
#  executeCommand "scp /tmp/${ARCHIVE} ${SCP_BASE}:${WORKING_DIR}"
}
...
# Retrieve and explode the application archive
 ...
 executeCommand "tar -xvf ${Component}_${VERSION}.tar"
}
