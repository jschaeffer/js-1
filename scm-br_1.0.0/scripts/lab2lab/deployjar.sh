...
while getopts "ha:t:o:d:e:r:j:m:p:u:b:c:v" OPTION
do
    case $OPTION in
      a)
        APP=${OPTARG}
...
case $APP in
  old_dai-cip)
	...
	export SSH_BASE="ssh tcserver@${APPSERVER_IP}"	
        ...
        echo "Deploying DAI-CIP to Lab to Lab"
...
#        export JARCLNCMD="rm \-fR ${WORKING_DIR}"
        ...
        export APPSERVER_BIN="${APPSERVER_BASE}"
        executeCommand "scp retrieveApplicationArchive.sh ${SSH_CP_BIN_DIR}"
        # Currently not deliverying config to Lab2lab  - executeCommand "scp deployacp.sh ${SSH_CP_BIN_DIR}"
        executeCommand "${SSH_BASE} ${APPSERVER_BIN}/retrieveApplicationArchive.sh -a ${APP} -t ${TAR} -r ${VERSION}"
        # Currently not delivering config to Lab2lab - executeCommand "${SSH_BASE} ${APPSERVER_BIN}/deployacp.sh -a ${APP} -t ${TAR} -r ${VERSION} -s ${TARGENV}"
        executeCommand "${SSH_BASE} ${JARCPCMD}"
  ...
  metadata-sync)
        ...
        cd ${WORKING_DIR}
        echo "Running ./install.sh ${ADS_USER} ${CAAS_USER} against 10.13.17.130 1522 pro001"
        chmod +x install.sh
        ./install.sh ${ADS_USER} ${CAAS_USER} cm_system L2L0ra 10.13.17.130 1522 pro001 
        ;;
  MicroDev)
        export DESTBASE="/opt/tcserver/Microstrategy"
        export APPSERVER_IP="10.13.19.58"
        export WORKING_DIR="/tmp/deploymentWorkArea/${TAR}/${VERSION}"
        export JARCPCMD="\"cd ${WORKING_DIR}; /opt/Microstrategy/MicroStrategy/bin/mstrcmdmgr -n performance_test -u Administrator -p "Perf_test01" -f /tmp/deploymentWorkArea/${TAR}/${VERSION}/mstrcmdmgr.scp -o ${WORKING_DIR}/${VERSION}.log; unix2dos ${WORKING_DIR}/${VERSION}.log; cat ${WORKING_DIR}/${VERSION}.log\""
