...
while getopts "ha:t:d:f:e:i:s:j:p:r:u:v" OPTION
do
    case $OPTION in
      a)
        APP=${OPTARG}
...
executeCommand "./dropDb.sh -a ${APP} -t ${TAR} -i ${INSTANCE} -d ${purpose} -r ${VERSION} -e ${TARGENV} -j ${JDBC_URL} -p ${DB_PASS} -u ${DB_USER}"

if [[ $APP = "smsi-relay-client" ]]; then
  executeCommand "./retrieveApplicationArchive.sh -a ${APP} -t ${TAR} -r ${VERSION}"
  executeCommand "./dropDb.sh -a ${APP} -t ${TAR} -i ${INSTANCE} -d ${purpose} -r ${VERSION} -e ${TARGENV} -j ${JDBC_URL} -p ${DB_PASS}1 -u ${DB_USER}1"
  executeCommand "./retrieveApplicationArchive.sh -a ${APP} -t ${TAR} -r ${VERSION}"
  executeCommand "./dropDb.sh -a ${APP} -t ${TAR} -i ${INSTANCE} -d ${purpose} -r ${VERSION} -e ${TARGENV} -j ${JDBC_URL} -p ${DB_PASS}2 -u ${DB_USER}2"
fi
