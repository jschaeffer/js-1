...
export WORKING_DIR="${APP_INSTALLED}/App_Files"
       ...
       if [[ ${APP} = "dai-cip" ]]; then
         ...
         executeCommand "cd ${APP_HOME}/bin && tar -xvf ${WORKING_DIR}/${TAR}_${VERSION}.tar ${JarName1}.jar"
       else
         executeCommandNoFail "cd ${APP_HOME}; ./launcher.sh stop"
         executeCommand "cd ${APP_HOME}/${JarLoc1} && tar -xvf ${WORKING_DIR}/${TAR}_${VERSION}.tar ${JarName1}.jar"
#         executeCommand "cd ${APP_HOME}/${JarLoc1} && rm -fR lib && tar -xvf ${WORKING_DIR}/${TAR}_${VERSION}.tar lib; ls -al lib"
         executeCommand "cd ${APP_HOME}/${JarLoc1} && tar -xvf ${WORKING_DIR}/${TAR}_${VERSION}.tar lib; ls -al lib"
         executeCommand "cd ${APP_HOME}; ./launcher.sh start"
       fi
