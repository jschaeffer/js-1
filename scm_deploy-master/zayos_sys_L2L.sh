...
fnGetCompList() {
if [[ $Suite = "Base" ]]; then
  export Component="Dynamic-Ad-Insertion-engine Dynamic-Ad-Insertion-cm request-mgr-TWC request-mgr-CMC dai-cip-feedback smsi-publisher dai-smsi impression_collector ADI-MDI Sheringham"

 elif [[ $Suite = "FW" ]]; then
  export Component="Dynamic-Ad-Insertion-engine Dynamic-Ad-Insertion-cm dai-cip-feedback Pgmr-Cpgn-Int metadata-publisher smsi-publisher dai-smsi dai-smsi-relay smsi-msp-relay dai-national-cis impression_collector"

 elif [[ $Suite = "Google" ]]; then
  export Component="Dynamic-Ad-Insertion-engine Dynamic-Ad-Insertion-cm dai-cip-feedback Pgmr-Cpgn-Int smsi-publisher dai-smsi dai-smsi-relay smsi-msp-relay impression_collector"
...
case $Comp in
 ...
 request-mgr-TWC)
 if [[ $Suite = "Base" ]]; then
  inar[0]=Base:cv-l2ltwc-clstr5:TWC_REQMAN:request-manager:request-manager:REQMAN:REQMAN:-DCAMPAIGN_SCHEMA_NAME=CAAS40
 elif [[ $Suite = "FW" ]]; then
 ...
 request-mgr-CMC)
 if [[ $Suite = "Base" ]]; then
  inar[0]=Base:cv-l2lcmc-clstr5:CMC_REQMAN:request-manager:request-manager:REQMAN_CMC:REQMAN_CMC:-DCAMPAIGN_SCHEMA_NAME=CAAS40
 elif [[ $Suite = "FW" ]]; then
  echo "No $Comp installation for FW"
  exit
 elif [[ $Suite = "Cox" ]]; then
 ...
 ADI-MDI)
 if [[ $Suite = "Base" ]]; then
  inar[0]=Base:cv-l2lcmc-clstr4:CMC_ADI-MDI:ADI-MDI-Ingest:ADI-MDI-Ingest:ADI_MDI_CMC:ADI_MDI_CMC
 elif [[ $Suite = "FW" ]]; then
  echo "No $Comp installation for FW"
  exit
 elif [[ $Suite = "Cox" ]]; then
 ...
 if [[ $Suite = "Base" ]]; then
  ...
  inar[0]=FW:cv-l2lfw-clstr3:FW_MSP_RELAY:smsi-relay-msp:smsi-relay-msp
 elif [[ $Suite = "Cox" ]]; then
  echo "No $Comp installation for $Suite"
  exit
 elif [[ $Suite = "Google" ]]; then
  inar[0]=Google:cv-l2lggl-clstr3:GGL_MSP_RELAY:smsi-relay-msp:smsi-relay-msp
 elif [[ $Suite = "Broadway" ]]; then
  inar[0]=Broadway:cv-l2lbrd-clstr3:BRD_MSP_RELAY:smsi-relay-msp:smsi-relay-msp
 elif [[ $Suite = "Rentrak" ]]; then
  inar[0]=Rentrak:cv-l2lrtk-clstr3:RTK_MSP_RELAY:smsi-relay-msp:smsi-relay-msp
fi
 ;;
 dai-smsi-relay)
 if [[ $Suite = "Base" ]]; then
 ...
 if [[ $Suite = "Base" ]]; then
  ...
  inar[0]=FW:cv-l2lggl-clstr4:GGL_NCIS:dai-national-cis:nCisClient:NCIS_GGL:GOOGLE:-DCAAS_SCHEMA_NAME=CAAS_LOCAL
 elif [[ $Suite = "Broadway" ]]; then
  echo "No $Comp installation for $Suite"
  exit
 elif [[ $Suite = "Rentrak" ]]; then
 ...
 Sheringham)
 if [[ $Suite = "Base" ]]; then
  inar[0]=Base:cv-l2lcmc-clstr3:Sheringham
 elif [[ $Suite = "FW" ]]; then
  echo "No $Comp installation for Freewheel"
  exit
 elif [[ $Suite = "Google" ]]; then
  echo "No $Comp installation for Google"
  exit
 ...
 if [[ $Suite = "Base" ]]; then
  ...
  inar[1]=Base:::::ADS_CORE_COM:ADS_CORE_COM:"-DCAAS_SCHEMA_NAME=CAAS40 -Denvironment=prod"
 elif [[ $Suite = "FW" ]]; then
