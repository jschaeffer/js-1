...
fnClearLogs() {
  if [[ ${Clear_Logs} = "Yes" || ${targname} = "devint" ]] ; then
...
#--------------------------------------------------------------
 ...
 dai-etl-feeder)
       fnInitRpt
       export DependApps="oss_bar smsi dai-smsi-relay"
...
#--------------------------------------------------------------
 Dynamic-Ad-Insertion-engine)
    ...
    smsiCorrection)
       fnInitRpt
       export APP=smsi-correction-agent
       export APPSERVER_IP="$targname"
       export INSTANCE="smsiCorrection"
       export WEBAPPS="smsi-correction-agent"
       source /opt/build/scripts/ZayoSCMSysVars.sh
       fnVERSIONCheck
       fnDeployApp
       ;;
    ...
    smsi-msp-relay)
       ...
       export APPSERVER_IP="$targname"
       export INSTANCE="smsi-msp-relay"
    ...
    kafka_etl_consumer)
       ...
       export APP=
       export APPSERVER_IP="$targname"
       export INSTANCE="kafka_etl_consumer"
       export WEBAPPS=""
       source /opt/build/scripts/ZayoSCMSysVars.sh
       fnVERSIONCheck
       fnDeployApp
       ;;
    kafka_relay_consumer)
       fnInitRpt
       export APP=
       export APPSERVER_IP="$targname"
       export INSTANCE="kafka_relay_consumer"
       export WEBAPPS=""
       source /opt/build/scripts/ZayoSCMSysVars.sh
       fnVERSIONCheck
       fnDeployApp
...
#--------------------------------------------------------------
    ...
    dai-national-cis)
       ...
       export DB_PASS="NCIS"
       source /opt/build/scripts/ZayoSCMSysVars.sh
#LH 10/17/16: logic to differentiate (Spelling?) between branch 5.0.0 and subsequent branches for DB deployments.
       VERSIONCUT=`echo $VERSION | cut -d . -f 2 | tr -d ' '  | tr -d '\r'`
         if [ $VERSIONCUT = "0" ]; then
           echo "Branch 5.0.0 detected. Deploying database and application."
           fnVERSIONCheck
           fnStopApp
           fnDB_Rebuild
           fnDeployDB
           ...
           echo "Branch subsequent to 5.0.0 detected. Deploying application but not deploying database."
           fnVERSIONCheck
           fnDeployApp
         fi
       ;;
    ...
    ADI-MDI)
       fnInitRpt
       export APP="ADI-MDI-Ingest"
       export APPSERVER_IP="$targname"
       export INSTANCE="ADI-MDI"
       export WEBAPPS="ADI-MDI-Ingest"
       DB_PASS=ADIMDI
       DB_USER=ADIMDI
       source /opt/build/scripts/ZayoSCMSysVars.sh
       fnVERSIONCheck
    ...
    Sheringham)
     ...
     CERN)
       fnInitRpt
        export APP=$Component
        export APPSERVER_IP="$targname"
        export JarDirect="CERN"
        export JarLoc1="bin"
        export JarName1="cern-server"
        export ConfDir="conf"
        source /opt/build/scripts/ZayoSCMSysVars.sh
        fnDeployJar
       ;;
