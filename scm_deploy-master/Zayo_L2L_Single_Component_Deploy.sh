...
fnCpyScpts() {
  cd /opt/build/scripts/zayo_L2L
  executeCommand "scp /opt/build/scripts/zayo_L2L/stop.sh ${SSH_CP_BIN_DIR}"
  executeCommand "scp /opt/build/scripts/zayo_L2L/deployWar.sh ${SSH_CP_BIN_DIR}"
}
...
fnStopDepApp() {
#LYAM 4/29/16: This is what i believe is causing the failed ads-core deployments. I believe this function is saving the third dependent app as a third line that the deployDB then catches and tries to use.
#Solution 1: Use different naming convetion for counters "l/m" VS "i/k".
#Solution 2: unset variables at the end.
...
for Comp in $DependApps; do
 ...
 fnZayos_Sys
 for l in ${inar[@]}; do
    export System="$(echo ${inar[$m]} | cut -f2 -d:)"
    export Instance="$(echo ${inar[$m]} | cut -f3 -d:)"
    export Webapps="$(echo ${inar[$m]} | cut -f4 -d:)"
...
fnDeployApp() {
  ...
  fnDepWar
  fnCleanup
# Temporarily disabled due to rema-publsiher addition  fnRptVer
  fnPostToSlack
}
...
#  Uses a custom system definition due to it's temporary nature.  Will eventually be converted to the smsi-publisher section
    ...
    rema-publisher-CMC)
       ...
       fnInitRpt
       export k=0
       inar[0]=Base:cv-l2lcmc-clstr1:CMC_REMA_PUB:REQMAN_PUB:smsi-publisher:REMA_PUB_CMC:REMA_PUB_CMC
       for i in ${inar[@]}; do
        fnAssignREMA
        echo "$APPSERVER_IP $INSTANCE $WEBAPPS $APP $DB_USER $DB_PASS LiqBaOPTS=$LiqBaOPTS"
        if test "$DB_USER"; then
...
#  Uses the new array assignment method .vs. case logic
    request-mgr-TWC)
       ...
       source /opt/build/scripts/zayos_sys_L2L.sh
       fnZayos_Sys
       export Comp=request-mgr
       export Component=request-mgr
       for i in ${inar[@]}; do
        fnAssignInfo
        if test "$DB_USER"; then
         fnDeployDB
...
# -acp
    ...
    Sheringham)
       fnInitRpt
       export APP=$Component
       export APPSERVER_IP="cv-l2lcmc-clstr3"
